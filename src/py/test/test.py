...
def draw_bounding_squares(img, keypoints):
    for keypoint in keypoints:
        ...
        half_size = size // 2
        cv2.rectangle(img, (x - half_size, y - half_size), (x + half_size, y + half_size), (0, 255, 0), 2)
...
while True:    
    ...
    G_mask_with_blobs = cv2.drawKeypoints(G_mask, green_keypoints, np.array([]), (0, 0, 255),
    cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

    red_keypoints = red_blob_detector.detect(R_mask)
    R_mask_with_blobs = cv2.drawKeypoints(R_mask, red_keypoints, np.array([]), (0, 0, 255),
    cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
...
G_paramstring = G_h_max

with open("test.json", "w") as outfile:
    json.dump(G_paramstring, outfile)
