        ...
        with open(filename, 'r') as f:
            ...
            if len(txt) <= index:
                ...
                if len(txt2) == 2:
                    ...
                    value = default_value
    #if the file is not found or the index is out of range, set the value to the default value
...
# create the window "R_HSV" for the color red
cv2.namedWindow("R_HSV")

# opens the file "G_params.txt" and reads the trackbar positions
G_h_min = load_params_from_file("G_params.txt", "G HUE Min", "G_HSV", 0, default_value=0)
G_h_max = load_params_from_file("G_params.txt", "G HUE Max", "G_HSV", 1, default_value=179)
G_s_min = load_params_from_file("G_params.txt", "G SAT Min", "G_HSV", 2, default_value=0)
G_s_max = load_params_from_file("G_params.txt", "G SAT Max", "G_HSV", 3, default_value=255)
G_v_min = load_params_from_file("G_params.txt", "G VAL Min", "G_HSV", 4, default_value=0)
G_v_max = load_params_from_file("G_params.txt", "G VAL Max", "G_HSV", 5, default_value=255)

# opens the file "R_params.txt" and reads the trackbar positions
R_h_min = load_params_from_file("R_params.txt", "R HUE Min", "R_HSV", 0, default_value=0)
R_h_max = load_params_from_file("R_params.txt", "R HUE Max", "R_HSV", 1, default_value=179)
R_s_min = load_params_from_file("R_params.txt", "R SAT Min", "R_HSV", 2, default_value=0)
R_s_max = load_params_from_file("R_params.txt", "R SAT Max", "R_HSV", 3, default_value=255)
R_v_min = load_params_from_file("R_params.txt", "R VAL Min", "R_HSV", 4, default_value=0)
R_v_max = load_params_from_file("R_params.txt", "R VAL Max", "R_HSV", 5, default_value=255)
